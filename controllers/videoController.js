...
class Controller {
  ...
  async update(req, res, next) {
    try {
      ...
      if (id && req.user) {
        const oneRecord = await video.findOne({
          where: { id: id, user: req.user.id },
        });
        if (oneRecord) {
          const updatedRecord = await video.update(
            {
              name,
              manufacturer,
              pathimg,
    ...
    try {
      ...
      if (id && req.user) {
        ...
        if (oneRecord) {
          ...
          const updatedRecord = await video.update(record, {
            where: { id: id, user: req.user.id },
          });
  ...
  async getAll(req, res, next) {
    try {
      ...
      const getAllRecords = await video.findAndCountAll({
        where,
        limit,
        offset,
      });
  ...
  async getOne(req, res, next) {
    try {
      let { id } = req.params;
      const oneRecord = await video.findOne({ where: { id: id } });
